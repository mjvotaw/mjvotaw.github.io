...
export const NOTE_TYPE_LOOKUP_REV: Record<NoteType, string> = {
  Tap: "1",
  Hold: "2",
  Roll: "4",
  Mine: "M",
...
export class NotedataParserWithParity {
  serialize(notedata: Notedata, gameType: GameType, division: number): string {
    ...
    for (let measure = 0; measure < numMeasures; measure++) {
      ...
      for (let div = 0; div < division; div++) {
        ...
        while (
          ...
          if (note.parity) footPlacements = footPlacements + note.parity;
        }
        while (
        ...
        if (footPlacements.length > 0)
        {
          row.push(footPlacements)
        }
