...
export class Simfile {
  ...
  static fromNotesString(smString: string, type: "sm" | "ssc" = "sm")
  {
    let simfile = new Simfile();
    simfile.loadFromString(smString, type);
    return simfile;
  }

  constructor() { }
  
  public loadFromString(data: string, type: string)
  {
    //Remove Comments
    ...
    console.log(`Simfile::loadFromString: props=`, props);
    let ssc_pair = false
    ...
    for (const data of temp_charts) {
      ...
      {
        console.log(`Simfile::loadFromString: error=`, error);
        this.unloadedCharts.push(data)
        continue
      }
      console.log(`Simfile::loadFromString: data=`, data);
      this.addChart(chart)
    }
