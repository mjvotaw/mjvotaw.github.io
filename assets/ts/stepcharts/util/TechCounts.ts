  ...
  */
  ...
  if (rowIndex > 0 && previousRow && noteCount == 1 && previousNoteCount == 1) {
    for (const foot of FEET) {
      ...
      let previousColumn = previousRow.whereTheFeetAre[foot];
      let currentColumn = currentRow.whereTheFeetAre[foot];

      if (!previousRow.notes[previousColumn] || previousRow.notes[previousColumn].type != "Tap")
        {
          continue;
        }
      // Is this foot in the same place in both rows
      if (previousColumn == currentColumn)
      {
        // Make sure that this isn't just a foot holding a note across both rows,
        // and then make sure that the elapsed time is within the threshold
        if (
          !(previousRow.holds[previousColumn] && previousRow.holds[previousColumn].hold >= currentRow.beat) &&
          (
